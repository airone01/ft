...
export async function handler(interaction: ChatInputCommandInteraction) {
  ...
  try {
    ...
    embed
      ...
      .addFields(
        {name: 'Location', value: `<:Campus:1310905787044073533> ${user.campus.map(c => c.name).join(', ')}\n<:Location:1310643964545728602> ${(!locations || locations.length === 0) ? '???' : (locations[0].end_at ? 'Logged out' : (`${locations[0].host} - ${time(new Date(locations[0].begin_at), TimestampStyles.RelativeTime)}`))}`, inline: true},
        ...
        {name: 'Contact', value: `<:Name:1310906917727502376> ${user.login}\n<:Name:1310906917727502376> ${user.first_name} ${user.last_name}\n`, inline: true},
      );
