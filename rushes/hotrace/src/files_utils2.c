...
/*   Updated: 2025/02/23 19:18:03 by elagouch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_file_content	*init_file_content(void)
{
	...
	if (!content)
		return (NULL);
	content->lines = malloc(sizeof(char *) * GNL_INITIAL_CAPACITY);
	if (!content->lines)
	{
		free(content);
		return (NULL);
	...
	content->size = 0;
	content->capacity = GNL_INITIAL_CAPACITY;
	return (content);
}
...
int	expand_capacity(t_file_content *content)
{
	...
	new_capacity = content->capacity * GNL_GROWTH_FACTOR;
