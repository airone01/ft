...
int real_value(char *nbr, char *symbols, int baseValue, int position,
               int sign) {
  int i;
  ...
  if (!position) {
    ...
    while (*nbr != *(symbols + i))
      ++i;
    return (real_value(nbr + 1, symbols, baseValue, position - 1, sign) +
            (i * ft_pow(baseValue, position) * sign));
  }
}
