...
fn collectFileNamesExcludingEndsWith(b: *std.Build, dir_rel: []const u8, excludes: []const []const u8) []const []const u8 {
    const io = b.graph.io;
    var dir = b.build_root.handle.openDir(io, dir_rel, .{ .iterate = true }) catch return &.{};
    defer dir.close(io);
    var list: std.ArrayList([]const u8) = .empty;
...
fn addBinary(
    ...
    const is_root = dirExists(b, "milestone-4/cub3d/src");
    const prefix = if (is_root) "milestone-4/cub3d" else ".";
    ...
    exe.root_module.addCSourceFiles(.{
        .root = b.path(src_dir),
        .files = collectFileNamesExcludingEndsWith(b, src_dir, exclude_suffixes),
        .flags = c_flags,
    });
