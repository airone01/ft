...
/*   Updated: 2025/06/23 15:05:43 by elagouch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void set_horizontal_distance(t_data *data, t_ray *ray) {
  ...
  if (!ft_feq(ray->horizontal_x, DBL_MAX))
    ray->horizontal_dist = calculate_distance(
        player_x, player_y, ray->horizontal_x, ray->horizontal_y);
  else
...
static void set_vertical_distance(t_data *data, t_ray *ray) {
  ...
  player_x = data->player.pos_x;
  player_y = data->player.pos_y;
  if (!ft_feq(ray->vertical_y, DBL_MAX))
    ray->vertical_dist = calculate_distance(player_x, player_y, ray->vertical_x,
                                            ray->vertical_y);
  else
