...
bool add_sprite(t_data *data, double x, double y) {
  ...
  sprite->active = true;
  sprite->distance = 0;
  sprite->scale = (float)SPRITE_SCALE;
...
void init_sprite_system(t_data *data) {
  ...
  data->sprite_sys.sprites =
      ft_calloc((size_t)data->sprite_sys.capacity, sizeof(t_sprite));
  data->sprite_sys.render_order =
      ft_calloc((size_t)data->sprite_sys.capacity, sizeof(int));
    ...
    while (++i < data->sprite_sys.count) {
      ...
      while (++j < data->sprite_sys.sprites[i].animation.frame_count) {
        if (data->sprite_sys.sprites[i].animation.frames[j].img)
          mlx_destroy_image(
              data->mlx, data->sprite_sys.sprites[i].animation.frames[j].img);
      }
    }
...
void update_sprite_animations(t_data *data) {
  ...
  while (++i < data->sprite_sys.count) {
    if (data->sprite_sys.sprites[i].active) {
      update_sprite_animation(&data->sprite_sys.sprites[i], data->delta_time);
    }
  }
}
