...
/*   Updated: 2025/06/23 14:07:22 by elagouch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void calculate_sprite_transform(t_data *data, t_sprite *sprite,
  ...
  render->transform_x =
      render->inv_det * (data->player.dir_y * render->sprite_x -
                         data->player.dir_x * render->sprite_y);
  render->transform_y =
      render->inv_det * (-data->player.plane_y * render->sprite_x +
                         data->player.plane_x * render->sprite_y);
  render->sprite_screen_x =
      (int)((data->win_width / 2) *
...
static void calculate_sprite_dimensions(t_data *data, t_sprite *sprite,
  ...
  render->sprite_height = (int)full_scale_height * (int)sprite->scale;
  ...
  render->sprite_width =
      abs((int)(data->win_height / render->transform_y)) * (int)sprite->scale;
...
static void draw_sprite_column(t_data *data, t_sprite_render *render,
  ...
  tex_x =
      (int)(256 *
            (stripe - (-render->sprite_width / 2 + render->sprite_screen_x)) *
            texture->width / render->sprite_width) /
      256;
