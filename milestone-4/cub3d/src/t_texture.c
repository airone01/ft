...
static bool load_textures_ns(t_textures *textures, void *mlx_ptr,
  ...
  textures->north.img = mlx_xpm_file_to_image(
      mlx_ptr, north_path, &textures->north.width, &textures->north.height);
  if (!textures->north.img)
    return (false);
  textures->north.addr =
      mlx_get_data_addr(textures->north.img, &textures->north.bits_per_pixel,
                        &textures->north.line_length, &textures->north.endian);
  textures->south.img = mlx_xpm_file_to_image(
      mlx_ptr, south_path, &textures->south.width, &textures->south.height);
  if (!textures->south.img)
    return (false);
  textures->south.addr =
      mlx_get_data_addr(textures->south.img, &textures->south.bits_per_pixel,
                        &textures->south.line_length, &textures->south.endian);
  return (true);
}

static bool load_textures_ew(t_textures *textures, void *mlx_ptr,
                             char *east_path, char *west_path) {
  textures->east.img = mlx_xpm_file_to_image(
      mlx_ptr, east_path, &textures->east.width, &textures->east.height);
  if (!textures->east.img)
    return (false);
  textures->east.addr =
      mlx_get_data_addr(textures->east.img, &textures->east.bits_per_pixel,
                        &textures->east.line_length, &textures->east.endian);
  textures->west.img = mlx_xpm_file_to_image(
      mlx_ptr, west_path, &textures->west.width, &textures->west.height);
  if (!textures->west.img)
    return (false);
  textures->west.addr =
      mlx_get_data_addr(textures->west.img, &textures->west.bits_per_pixel,
                        &textures->west.line_length, &textures->west.endian);
  return (true);
}
