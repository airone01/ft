...
bool load_bonus_textures(t_data *data, char *door_path) {
  ...
  data->textures.door.img =
      mlx_xpm_file_to_image(data->mlx, door_path, &data->textures.door.width,
                            &data->textures.door.height);
  if (!data->textures.door.img)
    return (true);
  data->textures.door.addr = mlx_get_data_addr(
      data->textures.door.img, &data->textures.door.bits_per_pixel,
